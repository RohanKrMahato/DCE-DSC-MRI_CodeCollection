...
class AutoAifSel:
    ...
    def __init__(self, conc_fn, brain_mask_fn, outputDir = getcwd(), plot_config = 2, plot_suffix = '' ,  do_Mouridsen2006 = True, ROI_fn = ''):
        ...
        if do_Mouridsen2006:
            ...
            self.pre_proc()
            self.clustercount = 1
            self.k_means_preClust, self.concDatalowfM_preClust, self.concData2DNorm_preClust, self.ROI_preClust, _ = self.clustering()
            self.k_means_finalClust, self.concDatalowfM_finalClust, self.concData2DNorm_finalClust, self.ROI_finalClust, self.clusterOrder_finalClust = self.clustering()
            ...
            self.pre_proc()
            self.clustercount = 1
            self.ROI_preClust = self.ROI3 #Since we only cluster once, ROI3 will be equivalent with ROI_preClust in Mouridsen2006
            self.k_means_finalClust, self.concDatalowfM_finalClust, self.concData2DNorm_finalClust, self.ROI_finalClust, self.clusterOrder_finalClust = self.clustering()
            self.calc_aif()
    ...
    def calc_b_idx(self):
        ...
        conc_data_finite[np.isfinite(self.conc_data)] = self.conc_data[np.isfinite(self.conc_data)]
        conc_mean = np.sum(conc_data_finite, axis = (0, 1, 2))
            
        for idx in range(0,len(conc_mean)):
            diff1 = (conc_mean[idx]-conc_mean[idx+1])/(conc_mean[idx]-np.max(conc_mean))
            diff2 = (conc_mean[idx]-conc_mean[idx+2])/(conc_mean[idx]-np.max(conc_mean))
    ...
    def calc_features(self, ftype):
        ...
        if ftype == 'firstM_feat':
            timeVec = range(1,np.size(self.conc_data,3)-self.b_idx+1) #Tidsvektor med N-b_idx element
        ...
        feature_z[self.brain_mask_data*finiteMask] = stats.zscore(feature[self.brain_mask_data*finiteMask])
        return feature_z

    def calc_peakconc(self):
        idx_max = np.where(self.conc_data == np.amax(self.conc_data[np.isfinite(self.conc_data)]))[0][0]
        peakconc_data = np.mean(self.conc_data[:,:,:, self.b_idx+1:idx_max+3], 3)
        return peakconc_data
 
    def remove_non_finite(self):
        self.peakconc_data[np.isfinite(self.peakconc_data) == False] = 0 #This is for displaying only, Nans makes it look wierd.
    ...
    def remove_irrelevant_vxls(self):
        ...
        self.ROI2 = ROI2
        self.ROI3 = ROI3
        self.ROI3_4D = np.repeat(ROI3[:,:,:, np.newaxis], np.size(self.conc_data,3), axis = 3)
    ...
    def pre_proc(self):
        ...
        self.concData2D = np.reshape(conc_data[self.ROI3_4D], (np.sum(self.ROI3), np.shape(self.conc_data)[3]))
    ...
    def clustering(self):
        ...
        summedrep = np.repeat(summed[:, np.newaxis], np.shape(concData2D[:, clustIdcs])[1], axis = 1)
        ...
        firstM_split = np.array_split(firstM_data[ROI], numClust) #Lika stora intervall av vektorn concDataNorm
        ...
        for n in range(0,len(intervals)-1):
            C_in[n, :] = np.mean(np.take(concData2DNorm,firstM_sort[intervals[n]:intervals[n+1]], axis = 0), axis = 0)
    ...
    def calc_aif(self):
        ...
        if self.do_Mouridsen2006:
            ...
            #
            clustIdcs = range(self.clustIdcs.start, self.clustIdcs.stop)
            sortIdx = np.argsort(np.sum(self.t4*self.concDatalowfM_finalClust[:, clustIdcs], axis = 1)) #Sort by first moment

            concDataLowest3 = self.concDatalowfM_finalClust[sortIdx[:3]] #3 Curves of the concentration data with lowest first moment
            highAUC_idx =  np.argmax(np.sum(concDataLowest3[:, clustIdcs], axis =1)) #The curve that has the highest AUC
            concDataLowest3Mean = np.mean(concDataLowest3,0)
            scaleFact = np.sum(concDataLowest3[:, clustIdcs][highAUC_idx])/np.sum(concDataLowest3Mean[clustIdcs]) #Scales the mean value to the curve with highest AUC
            concDataLowest3MeanScaled = concDataLowest3Mean*scaleFact #This is the final AIF
