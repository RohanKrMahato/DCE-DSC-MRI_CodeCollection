...
header = 'True values: K_trans = {} ml/100g/min, v_e = {} ml/100g (%), v_p = {} ml/100g (%).'.format(K_trans*100, v_e*100, v_p*100)
...
header = 'True values: K_trans = {} ml/100g/min, v_e = {} ml/100g (%), v_p = {} ml/100g (%).'.format(K_trans, v_e, v_p)
...
for i in range(len(F_ps)):
	header_TM     += ', K_trans(F_p = {}), v_e(F_p = {})'.format(F_ps[i]*100, F_ps[i]*100)
	header_ETM    += ', K_trans(F_p = {}), v_e(F_p = {}), v_p(F_p = {})'.format(F_ps[i]*100, F_ps[i]*100, F_ps[i]*100)
	header_twoCXM += ', K_trans(F_p = {}), v_e(F_p = {}), v_p(F_p = {}), F_p(F_p = {})'.format(F_ps[i]*100, F_ps[i]*100, F_ps[i]*100, F_ps[i]*100)
