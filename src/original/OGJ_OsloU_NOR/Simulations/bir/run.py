...
header = 'True values are: K_trans = {}, v_e = {}, v_p = {}, F_p = {}'.format(K_trans, v_e, v_p, F_p)
header += '\nThe columns are:'
header += '\ndt (s)'
header_TM = header
header_ETM = header
...
for BTT in BTTs:
	...
	header_ETM  += ', K_trans(BTT = {}), v_e(BTT = {}), v_p(BTT = {})'.format(BTT, BTT, BTT)
	header_2CXM += ', K_trans(BTT = {}), v_e(BTT = {}), v_p(BTT = {}), F_p(BTT = {})'.format(BTT, BTT, BTT, BTT)
...
for i in range(len(BTTs)):
	...
	frq1, C_aFFT1 = dofft(t0, twoCXM(t0, K_trans=0.07, F_p=[0.2, 0.4, 0.8][i], v_e=0.2, v_p=0.02))
	if fft1 is None:
