...
def gammaVariate(t, t_0=None, A=None, alpha=None, beta=None, y_max=None, t_max=None):
	...
	if not None in [t_0, alpha, beta]:
		...
		return y_max * t_max**(-alpha) * np.exp(alpha) * t**alpha * np.exp(-alpha*t/t_max)
	else:
...
def gammaVariateFit(t, y):
	...
	best  = np.sum(np.abs(gammaVariate(t, alpha=alpha_guess[0], y_max=y_max_guess[0], t_max=t_max_guess[0]) - y)**2)
		...
		for y_max in y_max_guess:
			for t_max in t_max_guess:
				for alpha in alpha_guess:
					this_diff = np.sum(np.abs(gammaVariate(t, alpha=alpha, y_max=y_max, t_max=t_max) - y)**2)
					if this_diff < best:
						best = this_diff
						alpha_best = alpha
						t_max_best = t_max
