...
def loadDicomDir(path, each=None, load=None):
	...
	if ((each is not None) and (load is not None)):
		printWarning('I am confused. Cannot use both each and load at the same time. Using only load.')
		each = None
	...
	time    = time[0] # assuming the time array is the same for all slices (as they should be)
	time    = time - time[0]
	...
	MRImageArguments = {
		...
		'FA': dicomObjectList[0].FlipAngle*np.pi/180., # in radians
		'TR': dicomObjectList[0].RepetitionTime/1000., # in seconds
	}
...
class DisplayFunctions:
	...
	def imshow(self, data_array, **kwargs):
		...
		# initiate the image
		image = ax.imshow(data_array[0,0], cmap=plt.get_cmap(cmap), interpolation='nearest', vmin=vmin, vmax=vmax)
